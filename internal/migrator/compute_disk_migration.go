...
func MigrateInstanceDisks(ctx context.Context, config *Config, instance *computepb.Instance, gcpClient *gcp.Clients) error {
	...
	isRunning := gcpClient.InstanceIsRunning(ctx, instance)
	...
	if isRunning {
		...
		if err := gcpClient.StopInstance(ctx, config.ProjectID, zone, instance.GetName()); err != nil {
	...
	for _, disk := range attachedDisks {
		if err := gcpClient.DetachDisk(ctx, config.ProjectID, zone, instance.GetName(), disk.GetDeviceName()); err != nil {
		...
		if err := gcpClient.AttachDisk(ctx, config.ProjectID, instance.GetZone(), instance.GetName(), disk); err != nil {
	...
	if isRunning {
		if err := gcpClient.StartInstance(ctx, config.ProjectID, instance.GetZone(), instance.GetName()); err != nil {
...
func IncrementalSnapshotDisk(ctx context.Context, config *Config, disk *computepb.Disk, gcpClient *gcp.Clients) error {
	...
	if err := gcpClient.CreateSnapshot(ctx, config.ProjectID, disk.GetZone(), disk.GetName(), snapshotName, kmsParams, nil); err != nil {
